...
#define SIMPLE_MACROS_ERROR_PRINT_CANT_ALLOCATE_MEMORY(p_bytes) \
    fprintf(stderr, SIMPLE_MACROS_ERROR_CSTRING("Unable to allocate %zu bytes of memory."), (p_bytes), __LINE__, __FILE__)
...
#define SIMPLE_MACROS_ERROR_FAIL_COND_V_MSG(p_condition, p_return_value, p_message) \
    if (p_condition){ \
        fputs(SIMPLE_MACROS_ERROR_CSTRING(p_message), stderr); \
        return (p_return_value); \
    }
